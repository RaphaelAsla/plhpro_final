...
def load_data(title):
    file_path = filedialog.askopenfilename(
        title=title, filetypes=[("Excel files", "*.xlsx")]
    )
    if not file_path:
    ...
    try:
        ...
        messagebox.showerror(
            "Σφάλμα", f"Σφάλμα κατά την ανάγωνση του αρχείου Excel: {file_path}"
        )
        return None
    except Exception as e:
        messagebox.showerror(
            "Σφάλμα", f"Προέκυψε σφάλμα κατά τη φόρτωση του αρχείου {str(e)}."
        )
        return None
...
def load_past_campaign_data():
    ...
    if past_campaign_data is None:
        messagebox.showerror(
            "Σφάλμα", "Τα δεδομένα της προηγούμενης καμπάνιας δεν φορτώθηκαν."
        )
        return None
...
def save_predictions(df):
    messagebox.showinfo(
        "Ενημέρωση", "Αποθήκευση προβλεπόμενης ανταπόκρισης πελατών νέας καμπάνιας."
    )
    try:
        ...
        if os.path.exists(save_path):
            overwrite = messagebox.askyesno(
                "Προσοχή", "Θέλετε να αντικαταστήσετε το υπάρχον αρχείο;"
            )
            if not overwrite:
                messagebox.showwarning(
                    "Σφάλμα", "Δεν ήταν δυνατή η αποθήκευση των προβλέψεων."
                )
        if not save_path:
...
def on_train():
    ...
    messagebox.showinfo(
        "Ενημέρωση", "Η εκπαίδευση του μοντέλου πρόβλεψης ολοκληρώθηκε!"
    )
...
def on_predict():
    ...
    messagebox.showinfo(
        "Ενημέρωση", "Η πρόβλεψη ανταπόκρισης πελατών νεάς καμπάνιας ολοκληρώθηκε!"
    )
...
btn_predict = tk.Button(
    button_frame,
    text="Πρόβλεψη Ανταπόκρισης Νέων Πελατών",
    font=("Arial", 12),
    bg="#41b130",
    fg="white",
    width=50,
